...
import { onValue, ref, db, getEmpleados, createPermiso, updatePermiso, type Permiso, type Empleado } from '@/lib/firebase';
...
export function PermisosManager() {
  ...
  useEffect(() => {
    const unsub1 = onValue(ref(db, 'permisos'), (snap) => {
      const raw: Record<string, Permiso> = snap.val() || {};
      setPermisos(Object.values(raw).sort((a, b) => b.fecha.localeCompare(a.fecha)));
    });
    const unsub2 = onValue(ref(db, 'empleados'), (snap) => {
      const raw: Record<string, Empleado> = snap.val() || {};
      setEmpleados(Object.values(raw).filter(emp => emp.activo !== false));
      setLoading(false);
    });
