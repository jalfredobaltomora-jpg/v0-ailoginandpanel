...
export function MonthlyIssues() {
  ...
  const copyMonthly = async (group: MonthGroup, key: string) => {
    ...
    const lines = ['Factory Buyer\tBuyer\tNo. Audit\tNo. Failure\tFail Rate %\tMeas Qty\tMeas Def\tMeas Rate %\tVis Qty\tVis Def\tVis Rate %'];
    data.forEach(f => {
      lines.push(`${f.factoryBuyer}\t${f.buyer}\t${f.totalAudit}\t${f.totalFail}\t${f.totalRate}\t${f.measQty}\t${f.measDef}\t${f.measRate}\t${f.visQty}\t${f.visDef}\t${f.visRate}`);
    });
    const t = group.totals;
    lines.push(`\n${t.factoryBuyer}\t${t.buyer}\t${t.totalAudit}\t${t.totalFail}\t${t.totalRate}\t${t.measQty}\t${t.measDef}\t${t.measRate}\t${t.visQty}\t${t.visDef}\t${t.visRate}`);
    try {
  ...
  const renderCell = (f: FactoryRow, field: keyof FactoryRow, index: number, isEditing: boolean) => {
    if (isEditing && (field === 'totalAudit' || field === 'totalFail' || field === 'measQty' || field === 'measDef' || field === 'visQty' || field === 'visDef')) {
      return (
        <input
          type="text"
          defaultValue={f[field] as number}
                          ...
                          <tbody>
                            {data.map((f, i) => (
                              <tr key={i} className={rowClass(isEditing)}>
                                ...
                                <td className="border-r border-border/50 px-2 py-1.5 bg-green-950/30">{renderCell(f, 'totalAudit', i, isEditing)}</td>
                                <td className="border-r border-border/50 px-2 py-1.5 bg-green-950/30">{renderCell(f, 'totalFail', i, isEditing)}</td>
                            ...
                            {total && (
                              <tr className="border-t-2 border-primary/50 bg-primary/10 font-bold">
                                ...
                                <td className="border-r border-border/50 px-2 py-2 text-left">{total.buyer}</td>
                                <td className="border-r border-border/50 px-2 py-2 bg-green-950/40">{total.totalAudit}</td>
                                <td className="border-r border-border/50 px-2 py-2 bg-green-950/40">{total.totalFail}</td>
                                ...
                                <td className="border-r border-border/50 px-2 py-2 bg-green-950/40">{total.measRate}</td>
                                ...
                                <td className="px-2 py-2 bg-green-950/40">{total.visRate}</td>
                              </tr>
                            )}
                          </tbody>
                        </table>
