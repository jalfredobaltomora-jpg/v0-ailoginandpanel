...
import { Upload, CheckCircle, FileDown, Sparkles, Edit, ClipboardCopy, Save, RotateCcw, AlertCircle, BarChart3, CalendarDays, ChevronLeft, ChevronRight } from 'lucide-react';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { get, ref, db, push, set } from '@/lib/firebase';
...
export function WeeklyIssues() {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      const matriz: string[][] = XLSX.utils.sheet_to_json<string[]>(sheet, { header: 1 });
      ...
      for (let i = 0; i < matriz.length; i++) {
        ...
        if (factoryBuyerVal.toUpperCase().includes('DASOLTEX')) continue;
            ...
            {recentRecords.length > 0 && (
              <div className="rounded-lg border border-border/50 bg-muted/10 p-4">
                <h4 className="flex items-center gap-2 text-sm font-semibold text-muted-foreground mb-3">
                  <Save className="h-4 w-4" /> Registros recientes
                </h4>
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {customStartDate && customEndDate
                        ...
                        : 'Semanda detectada automáticamente'}
                    </p>
                  </div>
                </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-slate-800">
                    ...
                    <th rowSpan={2} className="border-b border-r border-border px-3 py-2.5 text-left font-semibold text-white sticky top-0 bg-slate-800 z-10">Buyer</th>
                    <th colSpan={3} className="border-b border-r border-border px-3 py-2.5 text-center font-semibold text-white sticky top-0 bg-slate-800 z-10">FINAL - TOTAL</th>
                    <th colSpan={3} className="border-b border-r border-border px-3 py-2.5 text-center font-semibold text-white sticky top-0 bg-slate-800 z-10">FINAL - MEASUREMENT</th>
                    <th colSpan={3} className="border-b border-border px-3 py-2.5 text-center font-semibold text-white sticky top-0 bg-slate-800 z-10">FINAL - VISUAL</th>
                  </tr>
                  <tr className="bg-slate-700">
                    <th className="border-b border-r border-border px-3 py-2 text-xs font-medium text-green-300 sticky top-[41px] bg-slate-700 z-10">No. Audit</th>
                    <th className="border-b border-r border-border px-3 py-2 text-xs font-medium text-green-300 sticky top-[41px] bg-slate-700 z-10">No. Failure</th>
                    ...
                    <th className="border-b border-r border-border px-3 py-2 text-xs font-medium text-muted-foreground sticky top-[41px] bg-slate-700 z-10">Insp. Qty</th>
                    <th className="border-b border-r border-border px-3 py-2 text-xs font-medium text-muted-foreground sticky top-[41px] bg-slate-700 z-10">Def. Qty</th>
                    <th className="border-b border-r border-border px-3 py-2 text-xs font-medium text-green-300 sticky top-[41px] bg-slate-700 z-10">Defect Rate (%)</th>
                    <th className="border-b border-r border-border px-3 py-2 text-xs font-medium text-muted-foreground sticky top-[41px] bg-slate-700 z-10">Insp. Qty</th>
                    <th className="border-b border-r border-border px-3 py-2 text-xs font-medium text-muted-foreground sticky top-[41px] bg-slate-700 z-10">Def. Qty</th>
                    <th className="border-b border-border px-3 py-2 text-xs font-medium text-green-300 sticky top-[41px] bg-slate-700 z-10">Defect Rate (%)</th>
                  </tr>
                </thead>
                <tbody>
                  {factories.map((f, i) => (
                    <tr key={i} className="border-b border-border/50 hover:bg-muted/10 transition-colors">
                      ...
                      <td className="border-r border-border/50 px-3 py-2.5 text-left">{f.buyer}</td>
                      <td className="border-r border-border/50 px-3 py-2.5 bg-green-950/30 font-medium">{renderCell(i, 'totalAudit', f.totalAudit)}</td>
                      <td className="border-r border-border/50 px-3 py-2.5 bg-green-950/30 font-medium">{renderCell(i, 'totalFail', f.totalFail)}</td>
                      ...
                      <td className="border-r border-border/50 px-3 py-2.5 bg-green-950/30 font-medium">{f.measRate}</td>
                      ...
                      <td className="px-3 py-2.5 bg-green-950/30 font-medium">{f.visRate}</td>
                    </tr>
                  ))}
                  {totals && (
                    <tr className="border-t-2 border-primary/50 bg-primary/5 font-bold">
                      ...
                      <td className="border-r border-border/50 px-3 py-3 text-left">{totals.buyer}</td>
                      <td className="border-r border-border/50 px-3 py-3 bg-green-950/40">{totals.totalAudit}</td>
                      <td className="border-r border-border/50 px-3 py-3 bg-green-950/40">{totals.totalFail}</td>
                      ...
                      <td className="border-r border-border/50 px-3 py-3 bg-green-950/40">{totals.measRate}</td>
                      ...
                      <td className="px-3 py-3 bg-green-950/40">{totals.visRate}</td>
                    </tr>
                  )}
                </tbody>
              </table>
