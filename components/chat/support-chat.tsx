...
export function SupportChat({ requestId, username, onClose }: SupportChatProps) {
    ...
    if (request?.status === 'ai-active' && !userDataLoaded.current) {
      ...
      getUsuarioByUsername(username).then(async (u) => {
        if (u) {
          ...
          setEmpleadoData(emp);
          const name = emp ? `${emp.nombres} ${emp.apellidos}` : u.username;
          const greet = `Hola ${name}! Soy el asistente virtual de IT. He identificado tu cuenta en el sistema.\n\nPuedo ayudarte a recuperar tu usuario o restablecer tu contraseña. Que necesitas exactamente?`;
          setAiMessages(prev => [...prev, { id: 'ai-greeting', role: 'assistant', content: greet }]);
          setContext('awaiting_intent');
        } else {
          const greet = 'Hola! Soy el asistente virtual de IT. No he podido identificar tu usuario en el sistema. Un tecnico de IT revisara tu caso para ayudarte.';
          setAiMessages(prev => [...prev, { id: 'ai-greeting', role: 'assistant', content: greet }]);
          setContext('done');
        }
      });
  ...
  const handleSendMessage = async () => {
    ...
    if (request?.status === 'ai-active') {
      ...
      const dataReady = userData !== null;
      const notFound = userDataLoaded.current && !userData;

      if (notFound) {
        reply = 'No he podido encontrar tus datos en el sistema. Un tecnico de IT te ayudara en breve.';
        setContext('done');
      } else if (wantsBoth(message)) {
        reply = 'Por seguridad, solo puedo revelar un dato a la vez. Necesitas tu usuario o tu contraseña?';
        setContext('awaiting_intent');
      } else if (wantsUsername(message) && dataReady) {
        ...
        setContext('done');
      } else if (wantsPassword(message) && dataReady) {
        const name = empleadoData ? `${empleadoData.nombres} ${empleadoData.apellidos}` : '';
        ...
        if (wantsUsername(message) && dataReady) {
        ...
        if (generic) {
          reply = generic;
          if (/usuario|username|acceder|login|ingresar|olvide|olvido|recuperar|perdi|perdida|id|identidad/i.test(message)) {
            setContext('awaiting_intent');
          }
        } else {
          reply = dataReady
            ? 'En que puedo ayudarte? Puedo decirte tu usuario o restablecer tu contraseña.'
            : 'Un momento por favor, estoy consultando tus datos...';
        }
      }
