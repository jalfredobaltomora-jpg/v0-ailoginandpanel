...
Equipo: ${"tablet"===e.tipo?"Tablet":"Scanner"} | Fecha: ${e.mesInventario}`,{width:l,margin:1,color:{dark:"#000",light:"#fff"}},()=>c(!0))}),()=>{t=!1}},[e,o,l]);let m=async()=>{if(!d.current)return;let t=d.current.toDataURL("image/png"),i=[`Empleado: ${e.empleadoAsignado}`,`Nombre: ${o}`,`Serie: ${e.serialNumber}`,`Marca: ${e.marca||"-"}`,`Modelo: ${e.modelo||"-"}`,`Equipo: ${"tablet"===e.tipo?"Tablet":"Scanner"}`,`Fecha Inv.: ${e.mesInventario}`],a=window.open("","_blank");a&&(a.document.write(`<!DOCTYPE html><html><head>
